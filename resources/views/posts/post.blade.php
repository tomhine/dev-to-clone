<x-app-layout :title="$post->title">
    <x-slot:styles>@livewireStyles</x-slot>
        <div class="flex flex-col gap-2 w-full max-w-7xl">
            <article class="bg-white rounded-xl border flex flex-col max-w-3xl">
                @if ($post->banner_image_url)
                <img src="{{ asset('images/banners/' . $post->banner_image_url) }}" alt="" aria-hidden
                    class="rounded-t-md">
                @endif
